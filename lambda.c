...
lambda_term_t* lambda_bool(bool b) {
    if(b) {
        return Lambda("t", Lambda("e", "t"));
    } else {
        return Lambda("t", Lambda("e", "e"));
    }
}

lambda_term_t* Not(void) {
    return Lambda("b", Lambda("t", Lambda("e", Appl(Appl("b", "e")), "t")));
}

lambda_term_t* Not_applied(lambda_term_t* b) {
    return Lambda("t", Lambda("e", Appl(Appl(b, "e")), "t"));
}
...
lambda_term_t* Eqz(ml_term_t* n) {
    return Appl(Appl(n, Not()), lambda_bool(true));
}

lambda_term_t* Incr(void) {
    return Lambda("k", Lambda("f", Lambda("x", Appl(Var("f"), Appl(Appl("k", "f"), "x")))));
}

lambda_term_t* Incr_param(char* int_name) {
    return Lambda("f", Lambda("x", Appl(Var("f"), Appl(Appl(int_name, "f"), "x"))));
}

lambda_term_t* Decr(void) {
    return Lambda("n", Appl(lambda_bool(true), Appl(Appl("n",
    Lambda("c", Appl("c", Lambda("x", Lambda("y", Lambda("z", Appl(Appl("z", Incr_param("y")), "y"))))))),
    Lambda("z", Appl(Appl("z", lambda_int(0)), lambda_int(0))))));
}
...
lambda_term_t* EqZ(lambda_term_t* n, lambda_term_t* m) {
    return Appl(n, Lambda("x", Lambda("y", Appl(Appl(Eqz(), "x"),
                                                EqN("y", Appl(lambda_bool(false), m)), 
                                                EqN("x", Appl(lambda_bool(true), m))))));
}

lambda_term_t* LeqZ(lambda_term_t* n, lambda_term_t* m) {
    Appl(n, Lambda("x", Lambda("y", Appl(Appl(Eqz(), "x"),
                                         LeqN(Appl(lambda_bool(true), m), "y"), 
                                         LeqN("x", Appl(lambda_bool(false), m))))));
}

lambda_term_t* LtZ(lambda_term_t* n, lambda_term_t* m) {
    Appl(n, Lambda("x", Lambda("y", Appl(Appl(Eqz(), "x"),
                                         LeqN(Incr(Appl(lambda_bool(true), m)), "y"), 
                                         LeqN(Incr("x"), Appl(lambda_bool(false), m))))));
}
...
lambda_term_t* pattern_matching(built_in_pseudo_types_t type, lambda_term_t* t, lambda_term_t* C) {
    switch(type) {
        case L_LIST :
    ...
    switch(parsed_code->type) { // TODO implement float
        ...
        case DECLARE :
            if(parsed_code->content.declare.is_rec) {
                ...
                return Appl(Lambda(parsed_code->contenr.declare.var_name, code_gen(parsed_code->content.declare.in)), code_gen(parsed_code->content.declare.val));
            }
            res->it_content.it_decl.in = inter_code_gen(parsed_code->it_content.it_decl.in);
        case VARIABLE :
            return Var(parsed_code->content.var_name);
        case CONST_INT :
            if(parsed_code->content.n < 0) {
                return Lambda("z", Appl(Appl("z", lambda_int(0)), lambda_int(parsed_code->content.n)));
            } else {
                return Lambda("z", Appl(Appl("z", lambda_int(parsed_code->content.n)), lambda_int(0)));
            }
        case CONST_BOOL :
        ...
        case CONST_UNIT :
        case PARENTHESIS :
            return code_gen(parsed_code->content.parenthesis.inside);
        case COUPLE :
            return Appl(Appl("z", code_gen(parsed_code->content.cpl.fst)), code_gen(parsed_code->content.cpl.snd));
        case LIST :
        ...
        case CONDITION :
            return Appl(Appl(inter_code_gen(parsed_code->content.ite.condition), inter_code_gen(parsed_code->content.ite.body_i)), inter_code_gen(parsed_code->content.ite.body_e));
        case COMPARISON :
        ...
        case MATCH :
            switch(parsed_code->patterns[0]->type) { // TODO complete
                case LIST :
                case COUPLE :
                case VARIABLE :
                case CONST_INT :
